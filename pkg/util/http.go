...
// SerializeAlertProtoResponse serializes a protobuf response into an HTTP response.
func SerializeAlertProtoResponse(w http.ResponseWriter, resp proto.Message) error {
	data, err := proto.Marshal(resp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return fmt.Errorf("error marshaling proto response: %v", err)
