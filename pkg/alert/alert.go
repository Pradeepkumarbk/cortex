...
import (
	...
	prom_chunk "github.com/weaveworks/cortex/pkg/prom1/storage/local/alert"
...
func (c *Chunk) Decode(decodeContext *DecodeContext, input []byte) error {
	...
	if c.metadataInIndex {
		var err error
		c.Data, err = prom_chunk.NewForEncoding(prom_chunk.Simple)
		if err != nil {
			return err
		}
...
func chunksToMatrix(ctx context.Context, chunks []Chunk, from, through model.Time) (alertmodel.Matrix, error) {
	...
	sampleStreams := map[model.Fingerprint]*alertmodel.AlertStream{}
	for _, c := range chunks {
		ss, ok := sampleStreams[c.Fingerprint]
		if !ok {
			ss = &alertmodel.AlertStream{
			// Metric: c.Metric,
			}
		...
		ss.Values = util.MergeAlertSets(ss.Values, samples)
	}
	sp.LogFields(otlog.Int("sample streams", len(sampleStreams)))

	matrix := make(alertmodel.Matrix, 0, len(sampleStreams))
	for _, ss := range sampleStreams {
		matrix = append(matrix, &alertmodel.AlertStream{
			// Metric: ss.Metric,
			Values: ss.Values,
		})
	}
...
func (c *Chunk) Samples(from, through model.Time) ([]client.MessageTracker, error) {
