...
func (c *Store) Get(ctx context.Context, from, through model.Time, allMatchers ...*labels.Matcher) (alertmodel.Matrix, error) {
	// log.Print("inside chunk_store get method")
	if through < from {
	...
	// log.Print("before extractmetricnamematcherfrommatchers")
	//TODO: Moteesh Here
	return c.getMetricNameMatrix(ctx, from, through, allMatchers, "alert")
	...
	// }
	// log.Print("before getSeriesMatrix")
	...
	// return c.getSeriesMatrix(ctx, from, through, nil, nil)

}

func (c *Store) getMetricNameMatrix(ctx context.Context, from, through model.Time, allMatchers []*labels.Matcher, metricName string) (alertmodel.Matrix, error) {
	log.Print("inside getMetricNameMatrix")
...
func (c *Store) getMetricNameChunks(ctx context.Context, from, through model.Time, allMatchers []*labels.Matcher, metricName string) ([]Chunk, error) {
	log.Print("inside getMetricNameChunks")
	logger := util.WithContext(ctx, util.Logger)
	filters, matchers := util.SplitFiltersAndMatchers(allMatchers)
	log.Print("before calling c.lookupChunksByMetricName")
	chunks, err := c.lookupChunksByMetricName(ctx, from, through, matchers, metricName)
	if err != nil {
		return nil, err
	}
	log.Printf("no.of chunks before filter = %v", len(chunks))
	...
	log.Printf("no.of filtered chunks  = %v", len(filtered))
...
func (c *Store) getSeriesMatrix(ctx context.Context, from, through model.Time, allMatchers []*labels.Matcher, metricNameMatcher *labels.Matcher) (alertmodel.Matrix, error) {
	// Get all series from the index
	userID, err := user.ExtractOrgID(ctx)
	if err != nil {
		return nil, err
	}
	log.Print("Before GetReadQueries")
...
func (c *Store) lookupChunksByMetricName(ctx context.Context, from, through model.Time, matchers []*labels.Matcher, metricName string) ([]Chunk, error) {
	log.Print("inside lookupChunksByMetricName")
	userID, err := user.ExtractOrgID(ctx)
	if err != nil {
		return nil, err
	}
	...
	if len(matchers) == 0 {
		log.Print("inside no matchers")
		queries, err := c.schema.GetReadQueriesForMetric(from, through, userID, model.LabelValue(metricName))
		if err != nil {
			return nil, err
		}
		log.Print("after GetReadQueriesForMetric")
		entries, err := c.lookupEntriesByQueries(ctx, queries)
		if err != nil {
			return nil, err
		}
		log.Printf("after lookupEntriesByQueries entries = %v", len(entries))
...
func (c *Store) lookupEntriesByQueries(ctx context.Context, queries []IndexQuery) ([]IndexEntry, error) {
	log.Printf("inside lookupEntriesByQueries no.of qu = %v", len(queries))
