...
func (a awsStorageClient) QueryPages(ctx context.Context, query IndexQuery, callback func(result ReadBatch, lastPage bool) (shouldContinue bool)) error {
	log.Printf("inside awsStorageClient QueryPages tablename = %s hash = %s", query.TableName, query.HashValue)
	...
	//TODO: Moteesh - uncomment
	// if query.RangeValuePrefix != nil {
	// 	log.Printf("RangeValuePrefix = %v", string(query.RangeValuePrefix))
	...
	// 	log.Printf("RangeValueStart = %v", string(query.RangeValueStart))
	// 	input.KeyConditions[rangeKey] = &dynamodb.Condition{
	// 		AttributeValueList: []*dynamodb.AttributeValue{
	...
	log.Printf("query valueequal = %s", string(query.ValueEqual))
	// Filters
	if query.ValueEqual != nil {
		log.Printf("query valueequal not nil = %s", string(query.ValueEqual))
	...
	// log.Print("before a.queryRequestFn")
	...
	// log.Print("after a.queryRequestFn")
	for page := request; page != nil; page = page.NextPage() {
		pageCount++
		log.Printf("pageCount = %v", pageCount)
		response, err := a.queryPage(ctx, input, page)
		if err != nil {
			log.Printf("q.queryPage error %v", err)
			return err
		}
...
func (a awsStorageClient) queryPage(ctx context.Context, input *dynamodb.QueryInput, page dynamoDBRequest) (dynamoDBReadResponse, error) {
	...
	for backoff.Ongoing() {
		...
		log.Print("inside queryPage step5")
		if err != nil {
			log.Print("inside queryPage step6")
			...
			log.Print("inside queryPage step7")
			return nil, fmt.Errorf("QueryPage error: table=%v, err=%v", *input.TableName, err)
		}
		log.Print("inside queryPage step8")
		queryOutput := page.Data().(*dynamodb.QueryOutput)
		log.Printf("inside queryPage step9 items = %v", len(queryOutput.Items))
		return dynamoDBReadResponse(queryOutput.Items), nil
	}
	log.Print("inside queryPage step err")
