...
// storageClient implements alert.storageClient for GCP.
type storageClient struct {
	cfg       Config
	schemaCfg alert.SchemaConfig
	client    *bigtable.Client
}

// NewStorageClient returns a new StorageClient.
func NewStorageClient(ctx context.Context, cfg Config, schemaCfg alert.SchemaConfig) (alert.StorageClient, error) {
...
func (s *storageClient) NewWriteBatch() alert.WriteBatch {
...
func (s *storageClient) BatchWrite(ctx context.Context, batch alert.WriteBatch) error {
...
func (s *storageClient) QueryPages(ctx context.Context, query alert.IndexQuery, callback func(result alert.ReadBatch, lastPage bool) (shouldContinue bool)) error {
...
func (s *storageClient) PutChunks(ctx context.Context, chunks []alert.Chunk) error {
...
func (s *storageClient) GetChunks(ctx context.Context, input []alert.Chunk) ([]alert.Chunk, error) {
	chunks := map[string]map[string]alert.Chunk{}
	...
	for _, c := range input {
		...
		if _, ok := chunks[tableName]; !ok {
			chunks[tableName] = map[string]alert.Chunk{}
		}
		chunks[tableName][key] = c
	}

	outs := make(chan alert.Chunk, len(input))
	...
	for tableName := range keys {
		...
		for i := 0; i < len(keys); i += maxRowReads {
			...
			go func(page bigtable.RowList) {
				decodeContext := alert.NewDecodeContext()
	...
	output := make([]alert.Chunk, 0, len(input))
