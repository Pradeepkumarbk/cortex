...
// ValueLen is the length of a value in bytes.
const ValueLen = 4 * 1024

// DefaultEncoding can be changed via a flag.
var DefaultEncoding = Simple
...
// the series.
func (d *Desc) Add(s client.MessageTracker) ([]Chunk, error) {
	if d.C == nil {
...
type Chunk interface {
	...
	Add(sample client.MessageTracker) ([]Chunk, error)
	Clone() Chunk
...
func RangeValues(it Iterator, in metric.Interval) ([]client.MessageTracker, error) {
	result := []client.MessageTracker{}
	if !it.FindAtOrAfter(in.OldestInclusive) {
		return result, it.Err()
	}
	for !model.Time(it.Value().Timestamp).After(in.NewestInclusive) {
...
func addToOverflowChunk(c Chunk, s client.MessageTracker) ([]Chunk, error) {
...
func transcodeAndAdd(dst Chunk, src Chunk, s client.MessageTracker) ([]Chunk, error) {
...
func New() Chunk {
	log.Printf("DefaultEncoding = %v", DefaultEncoding)
	log.Printf("Simple = %v", Simple)
	// chunk, err := NewForEncoding(DefaultEncoding)
	chunk, err := NewForEncoding(Simple)
	if err != nil {
		panic(err)
	}
	return chunk
...
func NewForEncoding(encoding Encoding) (Chunk, error) {
	switch encoding {
	case Simple:
		return newSimpleEncodedChunk(d1, ValueLen, true, ChunkLen), nil
	// case Delta:
	// 	return newDeltaEncodedChunk(d1, 0, true, ChunkLen), nil
	// case DoubleDelta:
	// 	return newDoubleDeltaEncodedChunk(d1, 0, true, ChunkLen), nil
	// case Varbit:
...
type indexAccessingChunkIterator struct {
	len       int
	pos       int
	lastValue client.MessageTracker
	acc       indexAccessor
}
...
func (it *indexAccessingChunkIterator) Scan() bool {
	...
	it.lastValue = client.MessageTracker{
		Labels:    "",
		Timestamp: int64(it.acc.timestampAtIndex(it.pos)),
		Value:     string(it.acc.sampleValueAtIndex(it.pos)),
	}
...
func (it *indexAccessingChunkIterator) FindAtOrBefore(t model.Time) bool {
	...
	it.lastValue = client.MessageTracker{
		Labels:    "",
		Timestamp: int64(it.acc.timestampAtIndex(i - 1)),
		Value:     string(it.acc.sampleValueAtIndex(i - 1)),
	}
	return true
}
...
func (it *indexAccessingChunkIterator) FindAtOrAfter(t model.Time) bool {
	...
	it.lastValue = client.MessageTracker{
		Labels:    "",
		Timestamp: int64(it.acc.timestampAtIndex(i)),
		Value:     string(it.acc.sampleValueAtIndex(i)),
	}
	return true
}

// value implements Iterator.
func (it *indexAccessingChunkIterator) Value() client.MessageTracker {
	return it.lastValue
}
