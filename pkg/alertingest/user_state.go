...
func (us *userStates) getOrCreateSeries(ctx context.Context, label string) (*userState, model.Fingerprint, *memorySeries, error) {
	userID, err := user.ExtractOrgID(ctx)
	if err != nil {
	...
	state, ok = us.states[userID]
	if ok {
		fp, series, err = state.unlockedGet(label, us.cfg)
		if err != nil {
			log.Printf("getOrCreateSeries err 1 : %v", err)
			us.mtx.RUnlock()
	...
	fp, series, err = state.unlockedGet(label, us.cfg)
	log.Printf("getOrCreateSeries err 2 : %v", err)
...
func (u *userState) unlockedGet(label string, cfg *UserStatesConfig) (model.Fingerprint, *memorySeries, error) {
	metric := model.Metric{
		model.MetricNameLabel: (model.LabelValue)(label),
	}
	...
	if !u.canAddSeriesFor((model.LabelValue)(label), cfg) {
		u.fpLocker.Unlock(fp)
