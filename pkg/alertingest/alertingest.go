...
import (
	...
	cortex_chunk "github.com/weaveworks/cortex/pkg/alert"
...
func New(cfg Config, chunkStore ChunkStore) (*Ingester, error) {
	...
	if err := alert.DefaultEncoding.Set(cfg.ChunkEncoding); err != nil {
		return nil, err
	}
...
// Push implements client.AlertIngesterServer
func (i *Ingester) Push(ctx old_ctx.Context, req *client.WriteRequest) (*client.WriteResponse, error) {
	var lastPartialErr error
	log.Printf("Push - received messages : %v", len(req.Messages))
	messages := util.FromAlertWriteRequest(req)
	log.Printf("messages len = %v", len(messages))
	log.Printf("message 0 = %s, label = %s", messages[0].Value, messages[0].Labels)
	// messages := make([]client.MessageTracker, 0, len(req.Messages))
messages:
	for j := range messages {
		log.Printf("message = %s", messages[j].Value)
		if err := i.append(ctx, messages[j]); err != nil {
...
func (i *Ingester) append(ctx context.Context, message client.MessageTracker) error {
	...
	// 	}
	// }
	log.Print("inside alert ingester append method")
	i.stopLock.RLock()
	...
	log.Printf("message.Labels = %s", message.Labels)
	state, fp, series, err := i.userStates.getOrCreateSeries(ctx, message.Labels)
	if err != nil {
		log.Printf("alertingest append err 1 = %v", err)
		return err
	}
	defer func() {
	...
	if err := series.add(message); err != nil {
		log.Printf("alertingest append err 2 = %v", err)
		return err
	}
...
func (i *Ingester) Query(ctx old_ctx.Context, req *client.QueryRequest) (*client.QueryResponse, error) {
	start, end, matchers, err := util.FromAlertQueryRequest(req)
	if err != nil {
		return nil, err
	}
	...
	return util.ToAlertQueryResponse(matrix), nil
}

func (i *Ingester) query(ctx context.Context, from, through model.Time, matchers []*labels.Matcher) (alertmodel.Matrix, error) {
	i.queries.Inc()
	...
	err = state.forSeriesMatching(matchers, func(_ model.Fingerprint, series *memorySeries) error {
		...
		result = append(result, &alertmodel.AlertStream{
...
func (i *Ingester) MetricsForLabelMatchers(ctx old_ctx.Context, req *client.MetricsForLabelMatchersRequest) (*client.MetricsForLabelMatchersResponse, error) {
	...
	_, _, matchersSet, err := util.FromAlertsForLabelMatchersRequest(req)
	if err != nil {
		return nil, err
	}
	...
	return util.ToAlertsForLabelMatchersResponse(result), nil
}
