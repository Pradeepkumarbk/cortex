...
func (i *Ingester) flushChunks(ctx context.Context, fp model.Fingerprint, metric model.Metric, chunkDescs []*desc) error {
	log.Printf("inside flushChunks no of chunkdescs = %v", len(chunkDescs))
	userID, err := user.ExtractOrgID(ctx)
	if err != nil {
		return err
	}

	wireChunks := make([]alert.Chunk, 0, len(chunkDescs))
	for _, chunkDesc := range chunkDescs {
		wireChunks = append(wireChunks, alert.NewChunk(userID, fp, metric, chunkDesc.C, chunkDesc.FirstTime, chunkDesc.LastTime))
	}

	if err := i.chunkStore.Put(ctx, wireChunks); err != nil {
		log.Printf("error in chunkstore.put : %v", err)
		return err
	}
	...
	log.Print("successfully flushed chunks")
	return nil
}
