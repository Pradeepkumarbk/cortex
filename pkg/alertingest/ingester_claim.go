...
var (
	sentChunks = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cortex_alertingester_sent_chunks",
	...
	receivedChunks = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "cortex_alertingester_received_chunks",
...
func (i *Ingester) TransferChunks(stream client.AlertIngester_TransferChunksServer) error {
	...
	for {
		...
		// metric := util.FromAlertLabelPairs(wireSeries.Labels)
		...
		state, fp, series, err := userStates.getOrCreateSeries(userCtx, "alert")
		if err != nil {
			return err
		}
...
func toWireChunks(descs []*desc) ([]client.Chunk, error) {
	...
	for _, d := range descs {
		wireChunk := client.Chunk{
			...
			Data:             make([]byte, alert.ChunkLen, alert.ChunkLen),
		}
...
func fromWireChunks(wireChunks []client.Chunk) ([]*desc, error) {
	...
	for _, c := range wireChunks {
		...
		var err error
		desc.C, err = alert.NewForEncoding(alert.Encoding(byte(c.Encoding)))
		if err != nil {
			return nil, err
		}
